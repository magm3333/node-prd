...
function PRDReport(cfg, otherConfig) {
	...
	this.scriptsFolder=path.join(__dirname,'scripts');
...
PRDReport.prototype.getTempFilePath=function() {
	var fs = require('fs');
	var tmpPath=require('path').join(this.tmpParentFolder,'tmp');
...
PRDReport.prototype.getExecutable=function(initName) {
	...
	var executable=require('path').join(this.scriptsFolder,execFile);
	return executable;
};
...
PRDReport.prototype.runRaaS = function(cb) {
	...
	console.info("Try to run report in RaaS...");
	var prdT=this;
	try {
		var net = require('net');
		var socket = new net.Socket();
